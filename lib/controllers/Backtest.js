...
let BacktestController = class BacktestController extends odata_v4_server_1.ODataController {
    ...
    getById(key, query) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const db = yield connect_1.default();
            const backtest = new Backtest_2.Backtest(yield db.collection(collectionName).findOne({ _id }, { projection }));
            const { exchangeKey, currencyKey, assetKey, timeframe, start, end, indicatorInputs, strategyCode, initialBalance } = backtest;
            // добавить свечи
